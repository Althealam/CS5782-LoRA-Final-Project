...
LoRA exceeds our full-FT runs on QQP/SST-2/MRPC. We attribute this to lack of per-task LR/epoch tuning for full fine-tuning rather than a real inversion of the paper's findings; the original paper trains SST-2/QQP/MRPC for many more epochs and tunes LRs per task. Our ablations use a single seed per cell, so $\le 1$-point gaps should not be over-interpreted. Despite these caveats, every qualitative claim in the paper that we tested holds in our setup.
...
\bibitem{wolf2020transformers} T.~Wolf et al. \textit{Transformers: State-of-the-Art NLP}. EMNLP System Demos, 2020.
\end{thebibliography}
